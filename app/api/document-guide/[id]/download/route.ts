...
function forwardDownloadHeaders(from: Headers): Headers {
...
export async function GET(_request: Request, context: RouteContext) {
  try {
    ...
    const res = await beAuthenticatedFetch(
      `/document-guide/${encodeURIComponent(idPart)}/download`,
      { method: "GET" },
    );
    ...
    if (!res.ok) {
      ...
      return NextResponse.json({ message: message || "Download failed." }, { status: res.status });
    }

    if (res.body == null) {
      return NextResponse.json({ message: "Empty download response." }, { status: 502 });
    }

    return new NextResponse(res.body, {
      status: res.status,
      headers: forwardDownloadHeaders(res.headers),
    });
  } catch (e) {
